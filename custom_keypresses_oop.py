...
class ClickAndKeypressLogger:
    def __init__(self):
        ...
        self.box_buttons = {}
        self.box_text_map = {box_number: "" for box_number in range(1, 88)}
    ...
    def create_window(self):
        ...
        window.title("Click and Keypress Logger")

        label = ctk.CTkLabel(window, text="Click a box and press a key:")
        label.pack(pady=10)
        ...
        for box_number, box_name in box_names.items():
            box_button = ctk.CTkButton(window, text=box_name, command=lambda num=box_number: self.on_box_click(num))
            box_button.pack(pady=5)
            self.box_buttons[box_number] = box_button

        window.bind("<Key>", self.on_keypress)
        window.mainloop()
